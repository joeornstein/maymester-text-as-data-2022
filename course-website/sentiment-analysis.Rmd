...
For unsupervised learning models like [k-means](clustering.html) and [LDA](LDA.html), the objective is often discovery. We have a set of unlabeled data, and we want to get a sense of how we might organize the documents -- how we might sort them into buckets. But frequently social scientists turn to text data because we're interested in *measuring* some concept that is tough to quantify in other ways. For this, we'll want a different set of tools, because we have an objective and we want to build a model that satisfies that objective.
