...
Large language models (LLMs) have transformed the way computer scientists approach natural language processing over the past five years. These models are trained to predict the next word in a sequence, using a massive quantity of text data from the Internet and digitized books. In this tutorial, I will hand-wave over *how* exactly LLMs perform this task, focusing instead on how to adapt such models for use in social science applications. If you are interested in what goes on under-the-hood of an LLM, I highly recommend the following two blog posts for not-too-technical introdcutions:
...
If you prefer the model to auto-regressively generate text instead of outputting the next-word probabilities, set the `max_tokens` argument greater than 1. The function will return a character object with the most likely completion at each point in the sequence.
...
If we want GPT-3 to perform a classification task, we can structure the prompt so that the next-word prediction is the classification we want (a process called **adaptation**).
...
Next, we can format our prompt template using `format_prompt()`.
