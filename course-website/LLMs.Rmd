...
In the [OCR tutorial](OCR.html), we worked with a newspaper clipping about the Titanic sinking. 
...
Let's focus on reading the first column. Notice that while the text quality is generally legible, there are a number of lines where the text is cut off or blurred.
...
Next, we'll see how well OCR can capture the text from that image.
...
As is common with OCR, the result is very good on the first pass, but not perfect. See, for example, the phrase "reused tu recusiiee ce" (instead of "refused to recognize the").
...
**TODO**: Psych stuff from Text To Data.

## Text To Data

With a well-formatted prompt, we can 
...
The result (`response`), is a data table delimited by vertical bars, which we can then read into a data frame:
...
The `parse_text()` function puts all that together in a convenient interface.
