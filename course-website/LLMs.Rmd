...
Large language models (LLMs) have radically changed the way computer scientists approach natural language processing over the past five years. These models are trained to predict the next word in a sequence, using a massive quantity of training data from the Internet and digitized books. In this tutorial, I will hand-wave over how exactly they perform this task, and focus instead on how to adapt these models for use in social science applications. If you are interested in the details of how LLMs work, I found the following two blog posts particularly helpful:  

- [The GPT-3 Architecture, on a Napkin](https://dugas.ch/artificial_curiosity/GPT_architecture.html)

- [Large language models, explained with a minimum of math and jargon](https://www.understandingai.org/p/large-language-models-explained-with). 

In nutshell, LLMs represent words using [embeddings](word-embeddings.html), and predict the most likely next word in a sequence using a model architecture called the **transformer**. A powerful feature of this type of model is that the embeddings representing the input sequence are iteratively updated based on what words appear nearby (a process called **self-attention**). This allows an LLM to flexibly represent words based on their context, so that it knows, for instance, the difference between 

For our purposes here, it suffices to know that a **language model** is a model that predicts the most likely next word in a sequence of words. A **large** language model (LLM) is trained to perform this task using a massive corpus of data scraped from the Internet and digitized books. 

If we have a model that is sufficiently good at that task, we can *adapt* it to perform all sorts of text-as-data tasks. To show you how, will use the `text2data` R package to create LLM prompts and submit them to OpenAI's GPT-3.[^qualms] Let's start by getting the R package set up.

[^qualms]: Like many others, I have qualms about using closed-source models for scientific research [@spirlingWhyOpensourceGenerative2023], but as of summer 2023 the ease-of-use and capabilities of OpenAI's models are sufficiently beyond those of similar open-source models that it makes sense to start here. I will update both this page and the R package when I have identified a suitable set of open-source alternatives.
