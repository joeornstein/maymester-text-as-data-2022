...
```{r}
knitr::include_url('https://www.youtube.com/watch?v=mLyOj_QD4a4')
```

Note that the quality of these auto-generated transcripts can vary significantly, depending on audio quality. You can click the CC button in the bottom right of the video to get a sense of the quality of the closed captions.

To get the transcript of this video, we'll need the video ID, which you can find at the end of the video's URL (e.g. the bolded text here):
...
```{r}
knitr::include_url('https://www.youtube.com/watch?v=gzcBTUvVp7M')
```

```{r}
sotu_transcript <- get_youtube_transcript(video_id = 'gzcBTUvVp7M')

substr(sotu_transcript, 1, 1000)
```
