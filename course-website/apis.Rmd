...
We'll use the `glue` package to format web addresses:
...
Now we have a list called `content`. Sadly, this is not yet the content we want. The wrinkle here is that there are 8 versions of the text of this bill, from the first version when it was introduced in the House to the final version that was signed into law. The `content$textVersions` object tells us where to find each version of the text. 
...
Steps 1 through 4 compose the entire workflow for retrieving the full text of a bill from the Congress.gov API. If we ever plan to use that workflow again, it would be wise to encode it as a **function**. That way we're not copy-pasting large blocks of code every time we want to get the text of a new bill. Let's create a new function called `get_bill_text()`, which inputs the `congress`, `billType` and `billNumber`, goes through Steps 1-4, and returns the full text of the bill we want.
...
Play with the function a bit, changing the inputs to make sure that it returns the correct text when you ask for a different bill. And see the practice problems at the end of the page to get more practice working with APIs.

## YouTube API

In this tutorial, I'll show you how to get video transcripts from YouTube through its API. We'll be using the [youtube-transcript-api](https://pypi.org/project/youtube-transcript-api/) Python module, which lets us avoid all the work we just did formatting web addresses for the congress.gov API. All we need is the video ID of the YouTube transcript we want.

Oh. And we need to be able to run Python modules from `R`. Fortunately, the `reticulate` package is going to make that easy for us.
...
If you're on a Windows machine, just install the `reticulate` package, then use it to install Python on your machine, with the following two lines of code:
...
If you are a Mac user, you should follow the instructions for Windows PCs above, but then you're going to enter the following `R` command to create a Python "virtual environment".

```{r, eval=FALSE}
reticulate::conda_create('myenv')
```

Then restart your R session, then enter the following command to use the virtual environment you just created.
